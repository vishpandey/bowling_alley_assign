...
public class Pinsetter {
	 ...
	 * */
	private void sendEvent(int jdpins, String nick) {	// send events when our state is changd
		for (int i=0; i < subscribers.size(); i++) {
			((PinsetterObserver)subscribers.get(i)).receivePinsetterEvent(pins, foul, 
				throwNumber, jdpins, nick);
		}
	}
	 ...
	 */
	public void ballThrown(String nick, int throw_angle, int throw_velocity) {	// simulated event of ball hits sensor
		int count = 0;
		foul = false;
		...
		System.out.println("ball thrown and sending event");
		sendEvent(count, nick);
