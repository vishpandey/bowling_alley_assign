...
class LaneScore {
	...
	public void setScoresHashMap(HashMap scores) {
		this.scores = scores;
	}

	public void initScoresArrays(int size) {
		cumulScores = new int[size][14];
	}

	public int max_score(Bowler Cur) {
		int[] temp_scores = ((int[]) ((HashMap) getScoresHashMap()).get(Cur));
		int []high_score=new int[14];
		...
		for (int i=0;i<=31;i+=2)
		{
		   if(i!=18)
	       {
		   high_score[k]=temp_scores[i]+temp_scores[i+1];
		   }
		   else
		   {
		    high_score[k]=temp_scores[18]+temp_scores[19]+temp_scores[20];
			i++;
		   }
		   k++;
		}
	...
	public void markScore( Bowler Cur, int[] intArgs){
		...
		if(Cur.zeroes==2) {
			curScore[index-1]=((max_score(Cur))/2);
			Cur.zeroes=0;
		}
